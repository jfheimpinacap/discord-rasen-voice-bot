...
describe('guild command registration', () => {
  it('uses the guild-scoped route and central command definitions without HTTP', async () => {
    ...
    const definitions = createCommands({ configurationRepository: { findByGuildId: vi.fn(), upsert: vi.fn() }, logger: { error: vi.fn(), info: vi.fn(), warn: vi.fn() } }).toJSON();
    await registerGuildCommands({ put }, '100000000000000001', '100000000000000002', definitions);

    expect(put).toHaveBeenCalledWith(Routes.applicationGuildCommands('100000000000000001', '100000000000000002'), { body: definitions });
